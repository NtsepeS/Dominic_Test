...
RSpec.describe Modem, :type => :model do
  ...
  context 'executes methods correctly' do
    ...
    it 'returns parent core node' do
      expect(m.core_node).to eq(cn)
    end

    it 'returns parent base station unit' do
      expect(m.base_station_unit).to eq(bs1)
    end

    it 'returns parent base station sector' do
      expect(m.base_station_sector).to eq(sec)
    end

    it 'returns parent client link' do
      expect(m.client_link).to eq(cl)
    end
  end
end
